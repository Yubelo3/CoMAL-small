...
def active_sample_selection(model:dict, dataset:PubMedDataset, select=100, gamma=0.5, batch_size=64):
    ...
    prototypes=F.normalize(auto_encoder.mean[:-1],dim=-1).unsqueeze(0)  # [1 x C x sub_dim]
    with torch.no_grad():
    ...
    with torch.no_grad():
        for x in unlabeled_loader:
            ...
            similarity=(sub_rep*prototypes).sum(dim=-1)  # [B x C]
            positive_mask=(similarity>mean_similarity.unsqueeze(0)).float()  # [B x C]
    ...
    unselected_sample_index=[old_indices[x+old_len] for x in range(unlabeled_len) if x not in index]
